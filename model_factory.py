...
'''
Description: 
Projection: Video SuperResolution
Version: 1.0
Author: Gallery
Date: 2021-03-01 08:47:59
LastEditors: Gallery
Email: gaoyu@bupt.edu.cn
LastEditTime: 2021-03-16 22:39:20
'''
...
class Subpixel(Conv2D):
    '''
    description: implement of Subpixel
    param {*}
    return {*}
    author: Gallery
    '''
    ...
    def _phase_shift(self,I):
        ...
        ''' 
        r = self.r
        bsize,c,h,w = I.get_shape.as_list()
        bsize = K.shape(I)[0]     
class ESPCNN(Model):
    '''
    description: implement of espcnn
    param {*}
    return {*}
    author: Gallery
    '''
...
if __name__ == "__main__":
    ...
    print(HDimages.shape)#(50000, 32, 32, 3)
    ...
    for i, image in enumerate(HDimages):
        ...
        downscaled[i,:,:,:] = cv2.resize(image,None,fx=.5,fy=.5,interpolation=cv2.INTER_CUBIC)

    for i, image in enumerate(test_images):
        downscaled_test[i,:,:,:] = cv2.resize(image,None,fx=.5,fy=.5,interpolation=cv2.INTER_CUBIC)
    ...
    model.fit(padded, HDimages, epochs=100, validation_split=.2)
